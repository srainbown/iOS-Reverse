//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "HTTPResponse.h"

@class ALAssetRepresentation, ALAssetsLibrary, HTTPConnection, NSConditionLock;

@interface HTTPAlbumResponse : NSObject <HTTPResponse>
{
    HTTPConnection *connection;
    ALAssetRepresentation *_assetRep;
    ALAssetsLibrary *assetsLibrary;
    unsigned long long fileLength;
    unsigned long long fileOffset;
    void *buffer;
    unsigned int bufferSize;
    NSConditionLock *albumReadLock;
}

@property(retain, nonatomic) ALAssetRepresentation *assetRep; // @synthesize assetRep=_assetRep;
- (void)dealloc;
- (BOOL)isDone;
- (id)readDataOfLength:(unsigned int)arg1;
- (void)abort;
- (void)setOffset:(unsigned long long)arg1;
- (unsigned long long)offset;
- (unsigned long long)contentLength;
- (id)initWithAssetURL:(id)arg1 forConnection:(id)arg2;

@end

